...
<div class="text-[1.5em] xsm:text-[1em] relative z-[5]  h-full w-full pb-1hem pt-2em  font-bold font-bigShoulders uppercase">
    <div class="h-full w-[90%] mx-auto overflow-y-scroll pt-2em xsm:pt-1em thin-scrollbar ">
        <div class="relative w-[100%] xsm:w-[calc(100%-15em)] mx-auto flex">
            <div class="absolute flex flex-col gap-1em left-[2em] xsm:left-[2.6em]  ">
                {#each $postDataRanking.allLoadedData as rankers, i}
                    {#if showTiles}
                        <div 
                        in:fly={{y:10, delay:900+((i+1)*100)}}
                        class="">
                            <div class="text-[3.5em] xsm:aspect-[3/4] h-[2.85em] xsm:h-[1.72em] flex justify-center items-center">
                                <p>{i+1}</p>
                            </div>
                        </div>
                    {/if}
            ...
            <div class="flex flex-col gap-1em w-full">
                {#each $postDataRanking.allLoadedData as rankers, i}
                    {#if showTiles}
                        <div
                        in:fly={{y:10, delay:900+((i+1)*100)}}
                        style="order:-{$VoteDataRanking.votesArray[i] || 0}"
                        class="relative flex justify-center items-center h-[10em] xsm:h-[6em] gap-0em">
                            <div class="flex gap-[0.5em] items-center font-inter-italic absolute left-[calc(100%-3.6em)]  xsm:left-[calc(100%-3em)] text-[1.45em] bottom-[0em]">
                            ...
                            <!-- upvote Downvote -->
                            <div class="flex  flex-col xsm:flex-row gap-[0.5em] absolute right-[2.5em] xsm:right-[-2.2em] top-0">
                            ...
                            {#if isCreate}
                                <!-- svelte-ignore a11y_consider_explicit_label -->
                                <button on:click={()=> removeRow(i)} class="cursor-pointer w-[1.7em] xsm:w-[2.5em] absolute top-1/2 -translate-y-1/2 right-[0.5em] xsm:right-[-5.5em]">
                            ...
                            {/if}
                            <div class="bg-green-100 opacity-0 pointer-events-none hidden xsm:flex text-[3.5em] aspect-[3/4] h-full  justify-center items-center">
                                <p>{i+1}</p>
                            </div>
                            <button on:click|self={()=> create(i)}
                            class="ml-[1.8em] xsm:ml-0  p-[0.5em] cursor-pointer pb-[0.2em] uppercase h-full justify-between relative text-center bg-ranking-blocks flex  text-[1.25em] w-[calc(100%-10em)] mr-[2em] cursor-pointer">
                                        ...
                                        <button  class="flex ">
                                            ...
                                            {:else}
                                                <div class="  w-full flex h-auto  gap-1hem items-start pb-[0.4em] pr-[0.4em]">
                                                    ...
                                                    {/if}
                                                    <p class="whitespace-nowrap font-inter relative text-left translate-y-[-0.2em] text-[0.9em] grow">{rankers.teamName}</p>
                                                </div>
                                            {/if}
                                        </button>
                                    {:else}
                                ...
                                <div class="absolute bottom-[0.5em] xsm:bottom-[0.2em] right-[0.8em] font-inter flex justify-end items-end gap-1em">
                ...
                <!-- svelte-ignore a11y_consider_explicit_label -->
                {#if isCreate}
                    <div class="w-[calc(100%-12.4em)] ml-[6em] xsm:ml-[7.2em]">
