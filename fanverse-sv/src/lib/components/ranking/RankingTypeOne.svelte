...
<script>
    ...
    let dynamicWidth = "";
    $: if ($postDataRanking?.allLoadedData?.length) {
        dynamicWidth = $postDataRanking?.allLoadedData?.length > 0 
    }
    $:console.log("dynamicWidth", dynamicWidth);
    
</script>
    
<div class="text-[1.8em] overflow-scroll w-full xsm:text-[1em] relative z-[5] h-full  pb-4hem pt-2em  font-bold font-inter uppercase">
    <div class=" {dynamicWidth ? `w-[60em]`:"w-full"} xsm:w-full h-full">
        <div class="w-[95%] xsm:w-[90%] mx-auto">
            <div class="w-[calc(100%-6.2em)] xsm:w-[calc(100%-12.3em)] flex justify-end ml-[3.4em] xsm:ml-[6.8em] h-2em mb-1em ">
                <div class="pr-[1.5em] w-full text-[1.25em]">
                    <div class="w-full">
                        <div class="w-full flex justify-end items-center gap-1hem">
                            {#if isCreate}
                            ...
                            {#each $postDataRanking.allColumns as cols, i}
                                {#if isCreate}
                                <div style="width:{(cols.length * 0.58)}em"
                                ...
                                {:else}
                                <p
                                style="width:{(cols.length * 0.58)}em"
        ...
        <div class="h-full w-full overflow-auto thin-scrollbar ">
            <div class="w-[95%] xsm:w-[90%] mx-auto space-y-1em">
                {#each $postDataRanking.allLoadedData as rankers, i}
                    {#if showTiles}
                        <div in:fly={{y:10, delay:900+((i+1)*100)}}  class="relative flex justify-start items-center h-[5em] xsm:h-[4em] gap-2em">
                            {#if isCreate}
                                <!-- svelte-ignore a11y_consider_explicit_label -->
                                <button on:click={()=> removeRow(i)} class="cursor-pointer w-[2.5em] absolute top-1/2 -translate-y-1/2 right-0hem xsm:right-[1.5em]">
                                    <svg class="rotate-45" viewBox="0 0 58 58" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="29.0008" cy="28.9969" r="23.8344" stroke="white" stroke-width="1.2"/>
                                        <path d="M29 37.1172L29 20.8276" stroke="white" stroke-width="1.2" stroke-linecap="square"/>
                                        <path d="M37.1562 28.9922L20.8666 28.9922" stroke="white" stroke-width="1.2" stroke-linecap="square"/>
                            ...
                            {/if}
                            <div class="text-[2em] xsm:text-[1.7em] aspect-[2.8/10]  xsm:aspect-[5/4] h-full flex justify-center items-center">
                                <p>{i+1}</p>
                            </div>
                            <button
                            class="px-[0.5em] xsm:px-[1.5em] uppercase h-full justify-between relative text-center bg-pickem-box flex  text-[1.25em] w-[calc(100%-6em)] xsm:w-[calc(100%-10em)] border-l-[0.25em] border-pickem-title cursor-pointer">
                                <div class="flex w-[50%]">
                                ...
                                <div class=" flex justify-end items-center gap-1hem">
                                    {#each $postDataRanking.allColumns as cols, i}
                                        <!-- <p
                ...
                <!-- svelte-ignore a11y_consider_explicit_label -->
                {#if isCreate}
                    <div class="w-[calc(100%-7.3em)] xsm:w-[calc(100%-12.3em)] ml-[3.4em] xsm:ml-[7em]">
                        <button on:click={addRow} class="
                        pr-[1.5em] w-full text-[1em]
                        cursor-pointer opacity-75 hover:opacity-100  bg-pickem-box flex justify-start items-center h-[5em] xsm:h-[4em] gap-2em">
