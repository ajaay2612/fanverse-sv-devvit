<script>
    ...
    function addTeam() {
        ...
        $PostDataRanking.allLoadedData[$TeamPickerDataRanking.teamPickerIndex].teamName = teamData.teamName;
        $PostDataRanking.allLoadedData[$TeamPickerDataRanking.teamPickerIndex].teamImage = teamData.teamImage;
        ...
        $TeamPickerDataRanking.showTeamPicker = false;
    }

    onMount(() => {
        teamData = {
            teamName: $PostDataRanking.allLoadedData[$TeamPickerDataRanking.teamPickerIndex].teamName, 
            teamImage: $PostDataRanking.allLoadedData[$TeamPickerDataRanking.teamPickerIndex].teamImage
        } || teamData
    });
...
<div transition:fade={{duration:400, easing: cubicOut}} on:click|self={()=>$TeamPickerDataRanking.showTeamPicker = false} class="z-[150] backdrop-blur-[5px] text-[2.8em] bg-pickem-header-bg fixed left-0 top-0 h-full w-full  mx-auto flex flex-col xsm:flex-row justify-center items-center gap-3hem
    xsm:text-[1.5em]">
